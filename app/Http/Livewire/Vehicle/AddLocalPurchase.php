...
class AddLocalPurchase extends Component
{
    use WithFileUploads;
    public $size, $color, $color_code, $name,
        $vehicle_model_id, $opening_stock,
    ...
    public function render()
    {
        ...
        return view('livewire.vehicle.add-local-purchase', [
            'customers' => User::all(),
    ...
    public function updated($propertyName)
    {
        ...
        if ($propertyName == 'vehicle_condition') {
            if ($this->vehicle_condition == 'Used') {
                // If the 'vehicle_type' has a value, make 'chassis_id' nullable
                $this->rules['chassis_id'] = 'nullable';
            } else {
                // If 'vehicle_type' is empty, revert to the original validation rule
                $this->rules['chassis_id'] = 'required';
            }

            $this->validateOnly('chassis_id');
        }
    ...
    public function store()
    {
        ...
        LocalPurchase::create([
            ...
            'vehicle_price' => intval($this->vehicle_price),
        ...
        $this->dispatchBrowserEvent('success', ['msg' => 'Local Purchase Added Successfully']);
        return redirect()->route('vehicle.local.purchase.list');
    }
    //Package Trim
